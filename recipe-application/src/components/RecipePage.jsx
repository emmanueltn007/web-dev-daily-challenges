import { useEffect, useState } from "react";
import { useParams } from "react-router";

function RecipePage() {
  const { id } = useParams();
  const [recipe, setRecipe] = useState(null);
  const [isChecked, setIsChecked] = useState(new Set());

  useEffect(() => {
    fetch(`https://dummyjson.com/recipes/${id}`)
      .then((response) => response.json())
      .then((data) => {
        setRecipe(data);
      });
  }, [id]);

  function handleIngredientCheckList (index) {
    setIsChecked(prev => {
        const updated = new Set(prev);
        updated.has(index) ? updated.delete(index) : updated.add(index);
        return updated;
    });
  }

  if (!recipe) return <h1 className="px-8 py-8 lg:px-64 text-4xl font-thin">Loading...</h1>;

  return (
    <section className="px-8 py-8 lg:px-64">
      
      <div className="flex flex-col gap-2">
        <h1 className="text-4xl font-thin">{recipe.name}</h1>
        <div className="flex gap-4 text-lg lg:text-sm">
          <div className="flex items-center gap-1">
            <img src="../images/clock-icon.svg" alt="clock icon" />
            <span>{recipe.cookTimeMinutes}</span>
          </div>
          <div className="flex items-center gap-1">
            <svg
              className="h-6 text-orange-400"
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 640 640"
            >
              <path
                d="M256.5 37.6C265.8 29.8 279.6 30.2 288.4 38.5C300.6 50.1 311.7 62.9 322.3 75.9C335.8 92.4 352 114.2 367.6 140.1C372.8 133.3 377.6 127.3 381.8 122.2C382.9 120.9 384 119.5 385.2 118.1C393 108.3 402.9 96 416 96C429.4 96 438.8 107.9 446.8 118.1C448.1 119.8 449.4 121.4 450.7 122.9C461 135.3 474.7 153.2 488.4 175.3C515.6 219.2 544 281.7 544 351.9C544 475.6 443.7 575.9 320 575.9C196.3 575.9 96 475.7 96 352C96 260.9 137.1 182 176.5 127C196.4 99.3 216.2 77.1 231.1 61.9C239.3 53.5 247.6 45.2 256.6 37.7zM385.8 198.3C381.2 205.6 373 209.9 364.4 209.5C355.8 209.1 348 204.1 344.1 196.3C325.1 158.7 302.8 127.8 285.2 106.2C280 99.8 275.2 94.2 271 89.6C269.2 91.3 267.4 93.2 265.4 95.2C251.8 109.2 233.6 129.5 215.5 154.8C178.9 205.8 144 274.9 144 351.8C144 449 222.8 527.8 320 527.8C417.2 527.8 496 449 496 351.8C496 294.1 472.4 240.5 447.6 200.4C436.3 182.1 424.9 167 416 156C407.5 166.5 396.7 180.8 385.8 198.1zM321.7 480C258.5 480 208 439.4 208 370.8C208 323.3 241.2 284.3 259.5 266.5C265.8 260.4 275.6 261.5 281 268.4C297.2 288.9 329 329.3 346.3 351.4C352.5 359.3 364.4 359.6 371 351.9L396.2 322.6C402.7 315 414.2 315.7 418.7 324.6C444 370.8 432.7 429.6 390.6 459C369.5 473 347.1 480 321.8 480z"
                fill="currentColor"
              />
            </svg>
            <span>{recipe.caloriesPerServing}</span>
          </div>
          <div className="flex items-center gap-1">
            <img src="../images/star-icon.svg" alt="star icon" />
            <span>
              {recipe.rating}({recipe.reviewCount})
            </span>
          </div>
        </div>
      </div>

      <hr className="my-4 border-t border-gray-300" />
      
      <div className="w-full h-88 overflow-hidden rounded-md bg-center bg-no-repeat bg-cover" style={{ backgroundImage: `url(${recipe.image})`}}>
      </div>
      
      <div className="flex flex-col gap-4">
        <h4 className="text-2xl font-light">Ingredients</h4>

        <ol className="grid md:grid-cols-2 gap-2">
            {recipe.ingredients.map((ingredient, index) => {
            return (
                <li className="flex items-center gap-2 text-lg lg:text-sm" key={index}>
                    <button onClick={() => handleIngredientCheckList(index)} className="flex items-center justify-center h-5 w-5 border border-orange-400 rounded-full cursor-pointer">
                        <span className={`block h-4 w-4 rounded-full  ${isChecked.has(index) ? "bg-orange-400" : ""}`}></span>
                    </button>
                    <span className={isChecked.has(index) ? "line-through text-gray-500" : ""}>
                        {ingredient}
                    </span>
                    
                </li>
            );
            })}
        </ol>
      </div>
      

      <div className="flex flex-col gap-4">
        <h4 className="text-2xl font-light">Instructions</h4>

        <ol className="flex flex-col gap-2">
            {recipe.instructions.map((instruction, index) => {
            return (
                <li className="flex items-center gap-4 text-lg lg:text-sm" key={index}>
                    <span className="text-white text-xs bg-orange-400 w-6 h-6 md:w-5 md:h-5 p-1 rounded-full flex items-center justify-center">
                        {index + 1}
                    </span>
                    {instruction}
                </li>
            );
            })}
        </ol>
      </div>
    </section>
  );
}

export default RecipePage;
